...
자바스크립트 내장 타입 탐지 메커니즘
`typeof` : 데이터 타입 신뢰 불가능.
`instanceof` : 배열과 객체 혼동

- 객체가 네이티브 객체인지, 개발자가 정의한 객체인 지 알 수 없음.
...
- 이벤트 핸들러, `setTimeout()`, `setInterval()`에 사용.

##### 함수 커링 (Function Currying)

- 커링은 매개변수 일부가 미리 지정된 함수를 생성. '부분 함수 애플리케이션'이라고 불리기도 함.
- 생성된 새 함수를 호출할 때 매개변수 일부가 미리 지정됨.
- 일반적으로 다른 함수를 동적으로 호출하면서 커링할 함수와 매개변수를 전달하여 생성함.
...
기본적으로 자바스크립트의 객체는 확장 가능함. - 프로퍼티나 메서드 추가.
...
자바스크립트는 단일 스레드 환경에서 동작함. - 타이머는 단순히 코드를 나중에 실행하도록 미뤄두는 것임.
...
- 다른 인스턴스가 큐에 존재하지 않을 때만 추가됨.

단점

- 실행 되지 않는 ㄱ누간이 생길 수 있음.
...
- 배열 나누기 (**chunking**)
  ...
  - 처리할 일을 큐에 담고 다음에 할 일을 타이머로 예약하는 방법.
...
- 프로세스를 처리 단위 여러 개로 분리해서 큐에 넣음.
- 각 단위 사이 갭이 있으므로 다른 브라우저 프로세스를 이 시간에 처리함.

##### 함수 감속

DOM 조작은 다른 작업에 비해 메모리와 CPU 시간을 많이 요구함.
`onresize` 이벤트 - 타이머를 이용한 함수 감속으로 완화.
...
- 함수를 두번 째 호출하면 이전 타이머를 제거하고 다른 타이머를 생성.
- 이전 타이머가 이미 실행되었으면 연속 실행되지 않고, 이전 타이멎가 아직 실행되지 않았다면 새로운 타이머로 교체됨.
...
이벤트는 '옵저버'라는 디자인 패턴의 일종. 이 패턴은 코드를 느슨하게 연결하는 테크닉.

객체가 자신에게 어떤 의미가 있는 일이 일어났음을 나타나는 이벤트를 생성한다는 것.
다른 객체는 이 객체를 관찰하고 있닥다 이벤트에 응답하여 코드를 실행함.

- '주체' subject
- 옵저버 observer
  - 주체는 이벤트가 일어났음을 알리고, 옵저버는 이들 이벤트를 주시하며 주체를 관찰하기만 함.
  - 이 패턴의 핵심 개념은 주체가 옵저버에 대해 전혀 모른다는것.
  - 주체는 옵저버가 존재하던ㄴ 말던 자신의 역할을 수행.
  - 옵저버는 주체에 대해 알고 있으며 주체의 이벤트에 콜백(이벤트 핸들러)를 등록함.

DOM 요소가 주체, 이벤트 처리 코드가 옵저버임.
...
요소의 절대 위치를 지정하고 마우스로 움직일 수 있게함.
'커서 잔상' 이라는 고전적인 웹 트릭에서 출발. 마우스 포인터를 따라다니는 이미지.
...
이 기능을 마우스 버튼을 누를 때 등록 , 마우스 버튼을 놓을 때 제거
...
마우스 위치 개선
offsetLeft , offsetTop 프로퍼티 이용, diffX, diffY
